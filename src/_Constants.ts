...
export let PlaceholderCreator = {
    poleposition : (count : number, text : string) => { return `#${count + 1}${count * 2}`; },
    hexPlaceholder : (count : number, text : string) => { return `0x${(privateAreaNumber + count).toString(16)}`; },
    noEscape : (count : number, text : string) => { return text; },
    ninesOfRandomness : (count : number, text : string) => { return `9${Array.from({length: ninesLength}, () => Math.floor(Math.random() * 10).toString()).join("")}9`; },
    tagPlaceholderLetter : (count : number, text : string) => { return `<${String.fromCharCode(count + letterNumber)}>`; },
    tagPlaceholder : (count : number, text : string) => { return `<${(count + 1)}>`; },
    closedTagPlaceholder : (count : number, text : string) => { return `<${(count + 1)}/>`; },
    fullTagPlaceholder : (count : number, text : string) => { return `<${(count + 1)}></${(count + 1)}>`; },
    curlie : (count : number, text : string) => { return `{${(count + 1)}}`; },
    doubleCurlie : (count : number, text : string) => { return `{{${(count + 1)}}}`; },
    curlieLetter : (count : number, text : string) => { return `{${String.fromCharCode(count + letterNumber)}}`; },
    doubleCurlieLetter : (count : number, text : string) => { return `{{${String.fromCharCode(count + letterNumber)}}}`; },
    privateUse : (count : number, text : string) => { return `${String.fromCodePoint(count + privateAreaNumber)}`; },
    hashtag : (count : number, text : string) => { return `#${String.fromCharCode(count + letterNumber)}`; },
    hashtagTriple : (count : number, text : string) => { return `#${String.fromCharCode(count + letterNumber)}${String.fromCharCode(count + 1 + letterNumber)}${String.fromCharCode(count + 2 + letterNumber)}`; },
    tournament : (count : number, text : string) => { return `#${(count + 1)}`; },
    mvStyle : (count : number, text : string) => { return `%${count + 1}`; },
    wolfStyle : (count : number, text : string) => { return `@${count + 1}`; },
    wolfStyleLetter : (count : number, text : string) => { return `@${String.fromCharCode(count + letterNumber)}`; },
    percentage : (count : number, text : string) => { return `${(count + 1)}%`; },
    mvStyleLetter : (count : number, text : string) => { return `%${String.fromCharCode(count + letterNumber)}`; },
    sugoiTranslatorSpecial : (count : number, text : string) => { return `@#${count + 1}`; },
    sugoiTranslatorSpecial2 : (count : number, text : string) => { return `@#${String.fromCharCode(count + letterNumber)}`; },
}
